...
{
    public class AtualizarMetodoPagamentoRequestViewModelValidator : AbstractValidator<AtualizarMetodoPagamentoRequestViewModel>
    {
        public AtualizarMetodoPagamentoRequestViewModelValidator()
        {
            RuleFor(x => x.IdTipoMetodo)
                .InclusiveBetween(1, 5).WithMessage("O Id de tipo de conta deve estar entre 1 e 5.")
                .When(x => x.IdTipoMetodo.HasValue);
